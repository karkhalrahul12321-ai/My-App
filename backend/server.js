/* =========================================================
   PART 1/6 — BASE IMPORTS + CONFIG + SESSION + LOGIN
   ✅ PATCH: TOKEN LIFECYCLE FIX (NO LOGIC CHANGE)
========================================================= */
...
/* =========================================================
   🔐 PATCH START — TOKEN LIFECYCLE HELPERS
   (NEW, SAFE, DOES NOT AFFECT OTHER LOGIC)
========================================================= */

/** Check if REST session is still valid */
function isSessionValid() {
  return (
    session.access_token &&
    session.expires_at &&
    Date.now() < session.expires_at - 60_000 // 1 min buffer
  );
}

/** Guard for REST APIs (throws clean error) */
function ensureRestSession() {
  if (!isSessionValid()) {
    console.log("🔐 REST BLOCKED — SESSION EXPIRED", {
      now: Date.now(),
...
/* =========================================================
   SMARTAPI LOGIN (PATCHED: SAFE EXPIRY WINDOW)
========================================================= */
...
async function smartApiLogin(tradingPassword) {
  ...
  try {
    ...
    // ✅ PATCH: Safe expiry (do NOT trust 20h)
    session.expires_at = Date.now() + 6 * 60 * 60 * 1000; // 6 hours
...
/* =========================================================
   LOGIN ROUTES (UNCHANGED BEHAVIOR)
========================================================= */
...
module.exports = {
  ...
  smartApiLogin,
  generateTOTP,
  isSessionValid,      // 🔥 PATCH EXPORT
  ensureRestSession    // 🔥 PATCH EXPORT
};
/* =========================================================
...
async function tripleConfirmMomentum(symbol, getCandlesFn) {
  try {
    ...
    const pct = Math.abs(
      (last - meanPrev) / Math.max(1, meanPrev)
    );
...
/* MAIN OPTION LTP (CE / PE) — WS + REST HYBRID */
async function fetchOptionLTP(symbol, strike, type, expiry_days) {
  ...
  try {
    if (expiry_days === 0) {
      ...
      if (!ts.includes("CE") && !ts.includes("PE")) return null;
...
/* =========================================================
   ...
   ✅ PATCH: TOKEN LIFECYCLE SAFE GUARD
========================================================= */
...
/* =========================================================
   ...
   ✅ PATCH: SESSION EXPIRED HANDLING
========================================================= */
