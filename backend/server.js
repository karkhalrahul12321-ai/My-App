...
const SMART_API_SECRET = process.env.SMART_API_SECRET || ""; // अभी use नहीं हो रहा
...
// =====================================
// FALLBACK TOKENS (user से लिए हुए)
// =====================================
  ...
  try {
    ...
    const ltpVal =
      ...
      (Array.isArray(data.data) &&
        data.data[0] &&
        (data.data[0].ltp || data.data[0].last_traded_price)) ||
      null;
...
function buildStrikes(input, trend) {
  ...
  const spot = num(input.spot);
  const expiryDays = num(input.expiry_days, 7);
