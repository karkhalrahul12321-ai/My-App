...
function normalizeInput(body) {
  ...
  const marketRaw = (body.market || "nifty").toString().toLowerCase().trim();
...
function strikeEngine(input, trend) {
  ...
  // expiry के हिसाब से gaps थोड़ा adjust
  ...
  // Direction के हिसाब से CE/PE strikes चुनना
  ...
  // simple price model (फिलहाल demo)
  function buildOption(strike, type, distance) {
    // base premium सिर्फ demo formula
    const basePremium = Math.max(5, Math.round(distance / cfg.strikeStep) * 5);
  ...
  const ce = buildOption(
    ceStrike,
    "CE",
    Math.abs(ceStrike - spot)
  );

  const pe = buildOption(
    peStrike,
    "PE",
    Math.abs(peStrike - spot)
  );
