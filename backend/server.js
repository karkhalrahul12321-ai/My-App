...
/* =========================================================
   PART 4/6 — ENTRY ENGINE + FUTURES + OPTION LTP + TOKEN RESOLVER
   (ORIGINAL CODE — DO NOT MODIFY)
   ========================================================= */
...
/* =========================================================
   PART 6/6 — API ROUTES + CALC + SERVER START
   (ORIGINAL CODE — DO NOT MODIFY)
   ========================================================= */
...
app.get("/api/spot", async (req, res) => {
  try {
    ...
    if (
      ...
      lastKnown[market?.toLowerCase()]?.spot &&
      Date.now() - (lastKnown[market.toLowerCase()].updatedAt || 0) < 5000
    ) {
      return res.json({
        success: true,
        source: "WS",
        spot: lastKnown[market.toLowerCase()].spot
      });
    }
    ...
    if (market === "NIFTY" || market === "SENSEX") {
      ...
      lastKnown[market.toLowerCase()] = {
        spot: ltp,
        prevSpot: lastKnown[market.toLowerCase()]?.spot || null,
        updatedAt: Date.now()
      };
...
app.post("/api/calc", async (req, res) => {
  try {
    ...
    if (spot != null && isFinite(Number(spot))) {
      ...
      lastKnown[market?.toLowerCase()]?.spot &&
      Date.now() - lastKnown[market.toLowerCase()].updatedAt < 5000
    ) {
      finalSpot = lastKnown[market.toLowerCase()].spot;
    } else {
      const INDEX_MAP = {
        NIFTY: "NIFTY 50",
        SENSEX: "SENSEX"
    ...
    const entry = await computeEntry({
      ...
      rsi,
      expiry_days,
      lastSpot: lastKnown[market.toLowerCase()]?.prevSpot || null
    });
