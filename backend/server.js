/* PART 1/6 — BASE IMPORTS + CONFIG + SESSION + TOTP + LOGIN (FINAL FIXED) */
...
/* ===== SESSION (🔥 FINAL SHAPE) ===== */
let session = {
  access_token: null,   // jwtToken (REST)
  refreshToken: null,   // refresh
  feedToken: null,      // WS / feed
  expires_at: 0,
  login_time: null,

  // backward compatibility (अगर कहीं पुराना नाम use हो)
  refresh_token: null,
  feed_token: null
};
...
async function smartApiLogin(tradingPassword) {
  ...
  try {
    ...
    // 🔥 CRITICAL FIX — EXACT KEY NAMES
    session.access_token  = d.jwtToken || null;
    session.refreshToken  = d.refreshToken || null;
    session.feedToken     = d.feedToken || null;

    // backward compatibility
    session.refresh_token = session.refreshToken;
    session.feed_token    = session.feedToken;
...
async function resolveInstrumentToken(symbol, expiry = "", strike = 0, type = "FUT") {
  ...
  try {
    ...
    if (!Array.isArray(global.instrumentMaster) || !global.instrumentMaster.length) {
      ...
      const r = await fetch(url);
      const j = await r.json().catch(() => null);
      if (Array.isArray(j)) global.instrumentMaster = j;
    }
    ...
    if (type === "CE" || type === "PE") {
      ...
      const optList = marketCandidates.filter(it => {
        ...
        if (strike > 0 && Math.abs(st - strike) > 100) return false;
        return true;
      });
