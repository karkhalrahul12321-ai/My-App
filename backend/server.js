...
/* PART 3/6 — TREND + MOMENTUM + VOLUME + STRIKE ENGINE */
...
/* ================================
   🔥 STRIKE ENGINE (NEW, MODIFIED)
   ================================ */

/* Market-wise strike configuration */
const STRIKE_CONFIG = {
  NIFTY: {
    step: 50,
...
/* Expiry shrink ratio (A option – proportional shrink) */
function expiryShrinkRatio(days) {
  if (days >= 7) return 1;
...
/* FINAL STRIKE GENERATOR (REPLACES OLD generateStrikes) */
function generateStrikes(market, spot, expiry_days) {
  market = String(market || "").toUpperCase();
  const cfg = STRIKE_CONFIG[market];
  if (!cfg || !spot) return null;

  const { step, baseDistances } = cfg;

  // ATM calculation (market-wise step)
  const atm = Math.round(Number(spot) / step) * step;

  const ratio = expiryShrinkRatio(Number(expiry_days) || 0);

  const ce = [];
  const pe = [];

  for (const d of baseDistances) {
    const rawDist = d * ratio;
...
async function resolveInstrumentToken(symbol, expiry = "", strike = 0, type = "FUT") {
  try {
    ...
    const candidates = master.filter(it =>
      String(it.name || it.symbol || it.tradingsymbol || "")
        .toUpperCase()
        .includes(wantedSymbol)
    );
    ...
    if (wantedType === "CE" || wantedType === "PE") {
      ...
      const optList = candidates.filter(it => {
        ...
        const st = Number(it.strike || 0);
        return ts.endsWith(side) && Math.abs(st - wantedStrike) <= 100;
      });

      if (!optList.length) return null;

      optList.sort((a, b) =>
        Math.abs(parseExpiryDate(a.expiry) - Date.now()) -
        Math.abs(parseExpiryDate(b.expiry) - Date.now())
      );
    ...
    const futList = candidates.filter(it =>
