// =====================================
// Trading Helper Backend (FINAL STABLE + SEARCH FIX + DEBUG ROUTE)
...
// FRONTEND PATH (../frontend से index.html serve करेगा)
const frontendPath = path.join(__dirname, "..", "frontend");
app.use(express.static(frontendPath));

// NOTE: पहले सिर्फ "/" route; wildcard सबसे आख़िर में होगा
app.get("/", (req, res) => {
...
const SMART_API_SECRET = process.env.SMART_API_SECRET || ""; // अभी use नहीं हो रहा पर रखा है
...
// =====================================
// SMARTAPI SEARCH SCRIP (for auto token) – FIXED BODY
// =====================================
  ...
  try {
    const resp = await fetch(
      `${SMARTAPI_BASE}/rest/secure/angelbroking/order/v1/searchScrip`,
      {
        ...
        // ✅ SmartAPI forum के अनुसार सही body:
        // { "exchange": "NFO", "searchscrip": "BANKNIFTY..." }
        body: JSON.stringify({
    ...
    if (!data || !data.status || !Array.isArray(data.data)) return [];
    return data.data;
  } catch (err) {
...
// =====================================
// DEBUG TEST ROUTE – FULL RAW SEARCH
// =====================================
