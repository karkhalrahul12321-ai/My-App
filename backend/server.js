...
// ----- ROOT ROUTE FIX (frontend not present → safe HTML) -----
app.get("/", (req, res) => {
  res.type("html").send(`
    <h2>Trading Backend Running</h2>
    <p>Server is live and working.</p>
    <p>If you are seeing this, frontend build is not inside /public.</p>
    <p>Available test routes:</p>
    <ul>
      <li><a href="/api/ping">/api/ping</a></li>
      <li><a href="/api/ws/status">/api/ws/status</a></li>
    </ul>
  `);
});
