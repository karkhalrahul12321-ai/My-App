...
// =====================================
// FALLBACK TOKENS (तुम्हारे दिए हुए 3 tokens)
// =====================================
  ...
  try {
    const resp = await fetch(
      `${SMARTAPI_BASE}/rest/secure/angelbroking/order/v1/searchScrip`,
      {
        ...
        body: JSON.stringify({
          searchtext: keyword, // SmartAPI V2 correct param
        }),
      }
    );
...
async function autoFetchFuture(market) {
  ...
  for (const exp of expiries) {
    ...
    if (match) {
      ...
      console.log("AUTO FOUND:", AUTO[market]);
      return AUTO[market];
    }
  }
...
function autoDetectMarket(spot, explicit) {
  const m = (explicit || "").toString().trim().toLowerCase();
...
async function getAutoFutureLTP(market) {
  ...
  try {
    ...
    if (!data || data.status === false) {
      return { ok: false, reason: "LTP_FAILED", detail: data || null };
    }
...
// STRIKE ENGINE
// =====================================
function scaleDistancesByExpiry(expiryDays, base, step) {
...
function buildStrikes(input, trend) {
  ...
  return [
    ...
    makeOption(atm, "STRADDLE", atm - spot),
  ];
}
// =====================================
// MAIN API /api/calc
