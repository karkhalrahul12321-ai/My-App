...
/* ============================================================
   FUTURE RULES + AUTO TOKEN CACHE
============================================================ */
const FUT_RULES = {
  nifty:  { search: "NIFTY",       exch: "NFO", type: "FUTIDX", day: 2 },
...
const AUTO = {
  ...
  "natural gas":{ symbol: null, token: null, expiry: null, exch: "MCX", inst: "FUTCOM" }
};
...
/* ============================================================
   SMARTAPI SEARCH SCRIPT (for auto token)
============================================================ */
...
async function getFutureLTP(market) {
  ...
  // अगर token नहीं है तो पहले autoFetch से ले लो
  if (!auto.token) {
  ...
  try {
    const body = {
      mode: "LTP",
      exchangeTokens: {
        [rule.exch]: [auto.token]   // <<------ IMPORTANT FIX
      }
    };
    ...
    // SmartAPI v2 style: data.data?.fetched[0]?.ltp
    let ltp = null;
    if (data.data && Array.isArray(data.data.fetched) && data.data.fetched.length) {
      ltp = Number(data.data.fetched[0].ltp);
    } else if (data.data && typeof data.data.ltp !== "undefined") {
      ltp = Number(data.data.ltp);
    }
    ...
    console.log("LTP EXCEPTION:", e.message);
    return { ok: false, reason: "EXCEPTION", error: e.message };
  }
}

/* ============================================================
   MARKET CONFIG (STRIKES)
============================================================ */
const MRKT = {
  nifty: {
    step: 50,
...
function buildStrikes(input, trend) {
  ...
  const atm = roundToStep(num(input.spot), cfg.step);
  let ceDist, peDist;
