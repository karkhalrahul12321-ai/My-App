/* -------------------------------------------------------------
   ...
   FULLY SYNCED WITH FRONTEND FORMAT (NO LINES REMOVED)
...
async function fetchRecentCandles(symbol, interval, limit = 30) {
  try {
    // If interval = 1m AND websocket candles exist → use them
...
/* -------------------------------------------------------------
   ...
   (TRANSFORMED RESPONSE → frontend-friendly)
-------------------------------------------------------------- */
app.post("/api/calc", async (req, res) => {
  try {
    ...
    // Transform into frontend-friendly shape (without changing internal engine behavior)
    try {
      const meta = { live_data_used: !!(lastKnown.spot && Date.now() - (lastKnown.updatedAt||0) < 5000) };

      // build trend summary from returned compute (prefer returned trendObj, else recompute)
      const trend = {};
      ...
      // build strikes array expected by frontend (CE, PE, STRADDLE)
      const strikesArr = [];
      ...
      // STRADDLE (use otm1/otm2 as paired)
      strikesArr.push({
        type: 'STRADDLE',
        strike: s.atm,
      ...
      // fallback to original shape if transform fails
