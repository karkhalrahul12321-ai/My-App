...
// SmartAPI Login + Live FUT LTP + Trend + Strikes
...
const fetch = require("node-fetch"); // SmartAPI calls
...
// FUTURES TOKEN / SYMBOL (current front-month contracts)
// ये तुमने अभी env में जो डाले हैं, वही यहाँ connect हो रहे हैं
...
// अगर कभी exchange गलत हो तो env से override कर सकते हो
const SMART_NIFTY_EXCHANGE = process.env.SMART_NIFTY_EXCHANGE || "NFO";
const SMART_SENSEX_EXCHANGE = process.env.SMART_SENSEX_EXCHANGE || "BFO";
const SMART_NATGAS_EXCHANGE = process.env.SMART_NATGAS_EXCHANGE || "MCX";
...
const MARKET_CONFIG = {
  nifty: {
    ...
    exchange: SMART_NIFTY_EXCHANGE,          // default NFO
    futSymbol: SMART_NIFTY_SYMBOL,           // e.g. NIFTY30JAN25FUT
    futToken: SMART_NIFTY_TOKEN || ""        // e.g. 49546
  },
  sensex: {
    name: "Sensex",
    strikeStep: 100,
    baseDistances: { far: 500, mid: 400, near: 300 },
    exchange: SMART_SENSEX_EXCHANGE,         // default BFO (adjust via ENV अगर ज़रूरत हो)
    futSymbol: SMART_SENSEX_SYMBOL,          // e.g. SENSEX30JAN25FUT
    futToken: SMART_SENSEX_TOKEN || ""       // e.g. 49282
  },
  "natural gas": {
    name: "Natural Gas",
    strikeStep: 5,
    baseDistances: { far: 80, mid: 60, near: 50 },
    exchange: SMART_NATGAS_EXCHANGE,         // default MCX
    futSymbol: SMART_NATGAS_SYMBOL,          // e.g. NATURALGAS27JAN25FUT
    futToken: SMART_NATGAS_TOKEN || ""       // e.g. 474999
  }
};
...
async function getLiveLTP(tradingsymbol, exchange, symbolToken) {
  try {
    ...
    if (!data || data.status === false) {
      // यहाँ detail वापस कर रहा हूँ ताकि meta में दिख जाये SmartAPI क्या बोल रहा है
      return {
        ok: false,
    ...
    return { ok: true, ltp: data.data.ltp, raw: data };
  } catch (err) {
...
app.post("/api/ltp", async (req, res) => {
  try {
    ...
    res.json({ success: true, ltp: r.ltp, raw: r.raw || null });
  } catch (err) {
    res.json({
      success: false,
...
// =====================================
// /api/calc – MAIN API (FUT LTP + Trend + Strikes)
// =====================================
app.post("/api/calc", async (req, res) => {
  try {
    ...
    if (input.use_live && cfg && session.access_token) {
      const futSymbol = cfg.futSymbol || "";
      const futToken = cfg.futToken || "";
      const exchange = cfg.exchange || "NFO";

      const r = await getLiveLTP(futSymbol, exchange, futToken);
    ...
    res.json({
      ...
      meta: {
        ...
        live_error_detail: liveDebug || null,
        note:
