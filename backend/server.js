/* -------------------------------------------------------------
   ...
   FULLY SYNCED WITH FRONTEND FORMAT (NO CRITICAL LOGIC REMOVED)
   PART 1 / 10 — BASE IMPORTS + CONFIG + SESSION
-------------------------------------------------------------- */
...
const WebSocket = require("ws");     // <-- WebSocket Client (LIVE DATA)
...
/* -------------------------------------------------------------
   STRIKE GENERATOR (ATM + 2 OTM pair)
-------------------------------------------------------------- */
...
async function computeEntry({
  ...
  return {
    ...
    trendObj: trendObj,
    optionLtps: { ceATM, peATM, ceOTM, peOTM }
  };
}

/* -------------------------------------------------------------
   CANDLE FETCHERS (1m / 5m)
   - WebSocket 1m candles used when available; fallback to REST
-------------------------------------------------------------- */
...
// ----- Response Transform (Frontend Synced Format) -----
    try {
      const meta = { 
        live_data_used: !!(
          lastKnown.spot && Date.now() - (lastKnown.updatedAt || 0) < 5000
        )
      };

      // Build trend summary
      const trend = {};
      if (entry && entry.trendObj) {
        trend.main = entry.trendObj.direction || "NEUTRAL";
        trend.score = Number(entry.trendObj.score || 0);
        trend.strength = Math.abs(trend.score);
        trend.bias = entry.trendObj.direction || "NONE";
      } else {
        trend.main = entry.direction || "NEUTRAL";
        trend.score = 0;
        trend.strength = 0;
        trend.bias = entry.direction || "NONE";
      }

      // Build strikes array for frontend
      const strikesArr = [];
      const s = entry.strikes || { atm: null, otm1: null, otm2: null };

      // CE
      strikesArr.push({
        type: 'CE',
        strike: s.atm,
        distance: s.otm1 ? (s.otm1 - s.atm) : null,
        entry: entry.optionLtps ? entry.optionLtps.ceATM : entry.entryLTP || null,
        stopLoss: entry.sl || null,
        target: entry.target1 || null
      });
      ...
      strikesArr.push({
        type: 'PE',
        strike: s.atm,
        distance: s.otm1 ? (s.atm - s.otm2) : null,
        entry: entry.optionLtps ? entry.optionLtps.peATM : entry.entryLTP || null,
        stopLoss: entry.sl || null,
        target: entry.target1 || null
      });

      // STRADDLE (OTM pair summary)
      strikesArr.push({
        type: 'STRADDLE',
        strike: s.atm,
        distance: s.otm1 ? Math.abs(s.otm1 - s.atm) : null,
        entry: entry.entryLTP || null,
        stopLoss: entry.sl || null,
        target: entry.target1 || null
      });
      ...
      const respData = {
        ...
        data: {
          trend,
          strikes: strikesArr,
          input: { spot: finalSpot },
          meta,
          auto_tokens: { subscriptions: wsStatus.subscriptions || [] },
          login_status: session && session.access_token ? 'Logged-In' : 'Not logged-in (demo mode)'
        }
      };
      ...
      // fallback to original raw structure if formatting fails
...
/* -------------------------------------------------------------
   ...
   - This ensures Render opens your full UI (index.html)
   - Safe fallback added so server never crashes
-------------------------------------------------------------- */

// absolute path to "public" folder
const PUBLIC_DIR = path.join(__dirname, "public");

// If folder exists, serve it
if (fs.existsSync(PUBLIC_DIR)) {
  console.log("STATIC FRONTEND ENABLED:", PUBLIC_DIR);
  ...
  app.get("*", (req, res) => {
    ...
    res.send("Frontend missing: index.html not found, backend active.");
  });

} else {
  console.log("WARNING: public/ folder NOT FOUND. Frontend will not load.");

  // show backend message safely without breaking app
  app.get("*", (req, res) => {
    res.send("Backend OK — but frontend folder missing.");
  });
}

/* -------------------------------------------------------------
   FALLBACK ROOT (kept for compatibility)
-------------------------------------------------------------- */
app.get("/", (req, res) => {
...
/* -------------------------------------------------------------
   END OF FILE — RAHUL FINAL SYNCED BUILD
   - WebSocket LIVE
   - SmartAPI TOTP Login
   - Frontend fully served from /public
   - 100% frontend-backend sync
   - Render compatible (no crashes)
-------------------------------------------------------------- */
