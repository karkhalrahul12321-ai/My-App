...
async function startWebsocketIfReady() {
  ...
  wsClient.on("message", raw => {
    ...
    const sym = payload.tradingsymbol || payload.symbol || null;
    const itype = String(payload.instrumenttype || payload.instrumentType || "");

    if (!token || !isFinite(ltp) || ltp <= 0) return;
    ...
    if (itype.includes("INDEX") && ts.includes("SENSEX")) {
      lastKnown.sensex ??= {};
...
/* SUBSCRIBE CORE SYMBOLS — FINAL V2 DOC CORRECT */
...
async function subscribeCoreSymbols() {
  try {
    ...
    console.log("📡 WS SUBSCRIBED (V2)", tokenList);
...
/* MAIN OPTION LTP (CE / PE) — WS + REST HYBRID (FINAL & SAFE) */
async function fetchOptionLTP(symbol, strike, type, expiry_days, smartApi) {
  ...
  try {
    /* =====================================================
       1️⃣ EXPIRY RESOLVE (NO EXPIRED EXPIRY)
       ===================================================== */
    ...
    if (
      optionLTP[token] &&
      isFinite(optionLTP[token].ltp)
    ) {
      console.log(
        "⚡ OPTION LTP FROM WS CACHE",
        token,
        optionLTP[token].ltp
      );
    ...
    /* =====================================================
       5️⃣ GUARANTEED FALLBACK — ANGEL ONE REST
       ===================================================== */
    try {
      const url =
    ...
    /* =====================================================
       ...
       ===================================================== */
    console.log("⛔ OPTION LTP UNAVAILABLE", {
      symbol,
      strike,
      type,
      expiry,
