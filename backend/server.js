...
// =====================================
// SmartAPI docs: refresh/new JWT लेने के लिए यही API है:
...
async function refreshAccessToken() {
  ...
  try {
    ...
    if (d.refreshToken) {
      // कभी-कभी नया refreshToken भी आता है
...
// helper: जहां भी ज़रूरत हो, पहले इसे call कर सकते हैं
...
// =====================================
// SMARTAPI LOGIN (ORIGINAL v1 ENDPOINT)
// =====================================
    ...
    const resp = await fetch(
      `${SMARTAPI_BASE}/rest/auth/angelbroking/user/v1/loginByPassword`,
      {
        ...
        body: JSON.stringify({
          ...
          password: tradingPassword, // PIN या password
          totp: totp,
        }),
      }
    );
...
async function smartSearch(keyword) {
  ...
  // token expire हो चुका हो तो refresh try करो
  if (session.expires_at && session.expires_at < Date.now()) {
    const r = await refreshAccessToken();
    if (!r.ok) {
      console.log("TOKEN REFRESH FAILED BEFORE SEARCH:", r);
      // फिर भी नीचे try करेंगे, worst case error आ जाएगा
    }
  }
...
async function getAutoFutureLTP(market) {
  ...
  // token expire हो तो refresh try करो
  if (session.expires_at && session.expires_at < Date.now()) {
    const r = await refreshAccessToken();
    if (!r.ok) {
