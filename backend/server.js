...
// Market config
const MARKET_CONFIG = {
  nifty: { strikeStep: 50, angelSymbol: "NIFTY" },
  sensex: { strikeStep: 100, angelSymbol: "SENSEX" },
  "natural gas": { strikeStep: 5, angelSymbol: "NATURALGAS" }
};
...
function normalizeInput(body) {
  ...
  return {
    ...
    vwap: num(body.vwap),
    spot,
    market: autoDetectMarket(spot, body.market),
...
function computeTrend(input) {
  const emaDiff = input.ema20 - input.ema50;
  const emaPct = (emaDiff / ((input.ema20 + input.ema50) / 2)) * 100;

  let score = 50;
  score += clamp(emaPct, -20, 20);
  score += clamp(input.rsi - 50, -20, 20);

  const vwp = ((input.spot - input.vwap) / input.vwap) * 100;
  score += clamp(vwp, -10, 10);
...
function buildStrikes(input, trend) {
  ...
  return [
    { type: "CE", strike: atm + cfg.strikeStep },
    { type: "PE", strike: atm - cfg.strikeStep },
