...
/* ------------------------------------------------------------
   SERVE FRONTEND
   (keeps same structure as original; safe path resolution)
------------------------------------------------------------ */
...
/* ------------------------------------------------------------
   SmartAPI login (keeps original behavior; ensures session.login_time)
------------------------------------------------------------ */
...
/* ------------------------------------------------------------
   Login routes
   (single /api/login/status route kept — duplicates removed)
------------------------------------------------------------ */
...
/* ------------------------------------------------------------
   IMPORTANT: Keep a single root / fallback handler for now.
   (WebSocket & other APIs will be added in later parts)
------------------------------------------------------------ */
...
/* -------------------------------------------------------------
   EXPORT/START will be in Part-6 (so we don't start server twice
   while assembling parts). Keep this file segment purely definitional.
-------------------------------------------------------------- */
module.exports = {
  app,
  session,
...
async function startWebsocketIfReady() {
  ...
  wsClient.on("message", (raw) => {
    ...
    /* -------------------------------------------------------------
       BUILD 1-MIN CANDLE (corrected placement)
    -------------------------------------------------------------- */
    try {
      if (sym && ltp != null) {
...
/* -------------------------------------------------------------
   SUBSCRIBE CORE SYMBOLS (same logic)
-------------------------------------------------------------- */
...
/* -------------------------------------------------------------
   BASE TREND METRICS (unchanged logic)
-------------------------------------------------------------- */
...
/* -------------------------------------------------------------
   MOMENTUM TREND CHECKER (unchanged)
-------------------------------------------------------------- */
...
/* -------------------------------------------------------------
   RSI TREND FILTER (unchanged thresholds)
-------------------------------------------------------------- */
...
/* -------------------------------------------------------------
   STRIKE UTILS — ROUNDING + STEPS + DISTANCE
-------------------------------------------------------------- */
...
/* -------------------------------------------------------------
   FUTURES LTP FETCHER (unchanged logic)
-------------------------------------------------------------- */
...
/* -------------------------------------------------------------
   MAIN ENTRY ENGINE (FULL ORIGINAL LOGIC)
-------------------------------------------------------------- */
...
/* -------------------------------------------------------------
   FETCH HISTORICAL CANDLES (base REST call)
   - interval: "ONE_MINUTE" or "FIVE_MINUTE"
   - count used to build fromdate window (keeps same behavior)
-------------------------------------------------------------- */
...
/* -------------------------------------------------------------
   SIMPLE VOLUME SPIKE DETECTOR (non-blocking helper)
-------------------------------------------------------------- */
...
/* -------------------------------------------------------------
   FETCH LTP (SPOT) — REST fallback (used by /api/spot and calc)
-------------------------------------------------------------- */
...
/* -------------------------------------------------------------
   API: GET SPOT (LIVE → FALLBACK REST)
-------------------------------------------------------------- */
app.get("/api/spot", async (req, res) => {
  try {
    const market = String(req.query.market || "NIFTY").toUpperCase();

    // 1) LIVE WEBSOCKET spot (5 seconds freshness)
...
/* -------------------------------------------------------------
   API: RESOLVE INSTRUMENT TOKEN (OPTION / FUT)
-------------------------------------------------------------- */
