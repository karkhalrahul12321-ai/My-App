...
const SMART_API_SECRET = process.env.SMART_API_SECRET || ""; // if used
...
/*
  Note: These functions use Angel endpoints — adjust endpoint paths if your account/provider differs.
  They expect proper session.access_token to be set by login.
*/

async function fetchFuturesLTPForSymbol(symbol) {
  // Try multiple possible futures endpoints; return numeric or null
  try {
    if (!session.access_token) return null;
    const url = `${SMARTAPI_BASE}/rest/secure/angelbroking/marketdata/v1/getLTP`; // example endpoint; adapt if needed
    const body = { symbol }; // depends on API — keep adaptable
...
function findATMFromOptionChain(rawChain) {
  // rawChain assumed array of strikes with strikePrice & lastPrice & optionType
  if (!rawChain || !Array.isArray(rawChain) || !rawChain.length) return null;
  // find strike with highest total OI or nearest midpoint - simple heuristic: middle strike
...
function calcOCMidpointForATM(rawChain, atmStrike) {
  if (!rawChain || !Array.isArray(rawChain)) return null;
  const ce = rawChain.find((it) => Number(it.strikePrice) === Number(atmStrike) && (it.optionType === "CE" || it.optionType === "Call"));
  const pe = rawChain.find((it) => Number(it.strikePrice) === Number(atmStrike) && (it.optionType === "PE" || it.optionType === "Put"));
  if (!ce && !pe) return null;
  const cePrice = ce ? Number(ce.lastPrice || 0) : null;
  const pePrice = pe ? Number(pe.lastPrice || 0) : null;
  // infer underlying ATM from strike + mid of CE/PE? best effort: if both present use strike itself (we'll fallback)
  // Many brokers provide atmStrike info; otherwise we return null and upper layers will infer.
  return null;
}
...
/*
 Priority:
 1) If manual spot provided -> try verify with futures / OC
 2) If manual not provided -> if useLive permission -> fetch futures -> else option-chain -> else lastKnown
 3) Threshold based correction (default 0.25%)
*/
const SPOT_GUARDIAN_DEFAULT = {
  tolerancePct: 0.0025, // 0.25%
  cacheMaxAgeMs: 1000 * 60 * 5, // 5 min for lastKnown
};
...
async function spotGuardian(symbol, manualSpot, opts = {}) {
  ...
  // 1) If user provided, attempt verify
  if (user && user > 0) {
    // try futures
    ...
    if (fut && fut > 0) {
      ...
      if (diff / fut <= conf.tolerancePct) {
        ...
        // correction: prefer futures
        setUsed(fut, "futures_corrected", true, { manual: user, futures: fut });
        return result;
      }
    }
    // if futures not available, try option-chain ATM infer
    ...
    // If nothing available, accept manual as best-effort
    setUsed(user, "manual_best_effort", false, { manual: user });
    return result;
  }

  // 2) No manual -> try live futures if permission (opts.useLive true)
  if (opts.useLive) {
  ...
  // 3) try option-chain ATM inference
  ...
  // 5) ultimate fallback -> null (handled by caller)
...
async function fetchOptionLTPForStrike(symbol, strike, type) {
  ...
  try {
    ...
    if (!raw) return null;
    // raw might be array of objects: {strikePrice, lastPrice, optionType}
    const found = raw.find((it) => Number(it.strikePrice) === Number(strike) && ((it.optionType === "CE") || (it.optionType === "Call") ? type === "CE" : type === "PE"));
    if (found) {
      const l = Number(found.lastPrice || found.last || 0);
      return l > 0 ? l : null;
    }
    // fallback: try approximate strike match
...
/* ---------- Placeholder: small internal engines (if external modules missing) ---------- */
...
/* ---------- Full Suggestion Route (CORE OF ALPHA) ---------- */
app.post("/api/suggest", async (req, res) => {
  try {
    const { ema20, ema50, rsi, vwap, spot, expiry_days, market, useLive } = req.body || {};

    const mkt = (market || "").toUpperCase();
    const days = Number(expiry_days) || 7;

    // SPOT FIX USING SPOT-GUARDIAN
    const sFix = await spotGuardian(mkt, spot, { useLive: !!useLive });
    const finalSpot = sFix.spot_used;

    if (!isFinite(finalSpot)) {
      return res.json({ success: false, error: "Spot could not be resolved", guardian: sFix });
    }
    ...
    // VOLUME SPIKE (placeholder)
    const volumeSpike = detectVolumeSpike(1.2);

    // FUTURES DIFF (placeholder)
    ...
    // LTP + ENTRY CALIBRATION
    const fullList = [];

    for (let st of strikes) {
      const type = trendObj.main === "UP" ? "CE" : trendObj.main === "DOWN" ? "PE" : "CE";
