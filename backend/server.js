...
/* ============================
   PART 3/6 — TREND + MOMENTUM + VOLUME ENGINE
   ============================ */
...
async function tripleConfirmTrend(trendObj, symbol, getCandlesFn) {
  ...
  try {
    ...
    const localRSI =
      candles.length ? computeRSI(candles, 14) : null;
...
async function tripleConfirmMomentum(symbol, getCandlesFn) {
  try {
    ...
    const pct =
      Math.abs((last - meanPrev) / Math.max(1, meanPrev));

    let momentumConfirmed = pct > 0.0008;

    const downs1 = closes1
      .slice(-5)
      .every((v, i, arr) => (i === 0 ? true : arr[i] < arr[i - 1]));
    const ups1 = closes1
      .slice(-5)
      .every((v, i, arr) => (i === 0 ? true : arr[i] > arr[i - 1]));

    if (!(downs1 || ups1) && closes5.length >= 3) {
      const downs5 = closes5
        .slice(-3)
        .every((v, i, arr) => (i === 0 ? true : arr[i] < arr[i - 1]));
      const ups5 = closes5
        .slice(-3)
...
async function tripleConfirmVolume(symbol, getCandlesFn) {
  try {
    ...
    return {
      volumeConfirmed:
        latest >= Math.max(median * 0.9, mean * 0.8)
    };
  } catch {
...
/* ============================
   PART 4/6 — ENTRY ENGINE + TOKEN RESOLVER
   ============================ */

/* OPTION EXCHANGE DETECTOR */
...
async function fetchOptionLTP(symbol, strike, type, expiry_days) {
  try {
    ...
    if (apiLtp > 0) return apiLtp;

    const wsHit = optionLTP[tokenInfo.token];
    if (wsHit && wsHit.ltp > 0) return wsHit.ltp;
...
async function resolveInstrumentToken(
  ...
  try {
    ...
    const candidates = master.filter((it) => {
      ...
      return ts.includes(wantedSymbol);
    });
    ...
    if (wantedType === "CE" || wantedType === "PE") {
      ...
      const optList = candidates.filter((it) => {
        ...
        const isOption =
          itype.includes("OPT") || itype === "OPTIDX";

        if (!isOption) return false;
        if (!ts.includes(side)) return false;

        if (wantedStrike > 0) {
          if (Math.abs(st - wantedStrike) > 100) return false;
        }
      ...
      if (isTokenSane(picked.token)) {
        optionWsTokens.add(String(picked.token));
      }
    ...
    const futs = candidates
      .filter((it) => itypeOf(it).includes("FUT"))
      .map((it) => {
    ...
    console.log("resolveInstrumentToken ERR", e);
    return null;
  }
}
...
async function finalEntryGuard({
  ...
  if (passed === 0) {
    return { allowed: false, reason: "NO_CONFIRMATION" };
  }
...
/* FETCH RECENT CANDLES (REALTIME + REST) */
...
app.post("/api/calc", async (req, res) => {
  try {
    ...
    /* PRIORITY 3: REST FALLBACK */
      ...
      const fb = await fetchLTP(
        INDEX_MAP[market] || market
      );
...
/* FALLBACK ROOT */
app.get("/", (req, res) => {
  res.send("Backend OK — WebSocket Live 🚀");
});
